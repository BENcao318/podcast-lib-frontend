...
const AudioPlayer: React.FC<Props> = ({
  ...
  const episodePlayer = useAppSelector(
    (state: ReduxStateInterface) => state.episodePlayer
  )
  const queues = useAppSelector(
    (state: ReduxStateInterface) => state.queue.queues
  )
  ...
  const intervalRef: { current: NodeJS.Timer | number | undefined } = useRef()
  ...
  useEffect(() => {
    if (
      ...
      audioProgress >= audioRef.current.duration
    ) {
      unQueue()
